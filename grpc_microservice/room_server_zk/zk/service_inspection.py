...
from room_server.common_cls import Singleton
from room_server.zk.drama_zk_client import IKazooClient
from room_server.zk.zk_persistent_lock import zkPersistentLock
from room_server.zk.zk_server_content import ServerContent
...
class ServerInspecte(metaclass=Singleton):
    ...
    def server_transfer(self):
        ...
        if self.is_master:
            print("----------------触发服务转移----------------")
            from databases.models import GameRoom
            from common.project_key import ROOM_STATUS

            game_rooms = GameRoom.objects.filter(status__in=[ROOM_STATUS.GAMING.value, ROOM_STATUS.FINISH.value],
                                                 start_time__gt=datetime.datetime.now() + datetime.timedelta(
                                                     hours=-2)).exclude(
                server_node__in=list(ServerContent.get_list().keys()))
            # 房间没在
            for game_room in game_rooms:
                new_server_node = self.balance_strategy_cls.choice(ServerContent.get_list())
                print("房间 [%s]---node:%s  --->  node:%s" % (
                    game_room.room_number, game_room.server_node, new_server_node))
                game_room.server_node = new_server_node
                game_room.ws_path = settings.WS_PATH + ServerContent.get_websocket_path(game_room.server_node)
                game_room.mq_path = ServerContent.get_mq_path(game_room.server_node)
                game_room.save()
    ...
    def read_state(self):
        for node_name in ServerContent.get_list().keys():
            data = IKazooClient().get_etcd_client().get("/".join([self.BIZ_PATH, self.WEBSOCKET, node_name]))[0].decode(
                "utf-8")
    ...
    def read_servers(self):
        """获取服务列表"""
        node_list = IKazooClient().get_etcd_client().get_children("/".join([self.BIZ_PATH, self.WEBSOCKET]),
    ...
    def server_change_listener(self, event):
        # 服务检测  服务注册,节点状态存在延迟
        IKazooClient().get_etcd_client().get_children("/".join([self.BIZ_PATH, self.WEBSOCKET]),
                                                      watch=self.server_change_listener)
        self.read_servers()
    ...
    def create_master_node(self):
        zkPersistentLock().zk_async(IKazooClient().get_etcd_client(), "api_server", self.set_master, {},
                                    app_path="websocket")

    def choice_grpc_server(self,server_name):
        _server_root = '/GRPC'
        server_name = _server_root+server_name
        _children = IKazooClient().get_etcd_client().get_children(server_name)
        server_node = None
        for i in _children:
            v = IKazooClient().get_etcd_client().get("/".join([server_name, i]))
    ...
    def register_server(self,point,doc):
        zk = IKazooClient().get_etcd_client()
        _server_root = '/GRPC'
        point = point+"/"
