...
from grpc_microservice.room_server.zk_server.zk.drama_zk_client import IKazooClient
from grpc_microservice.room_server.zk_server.zk.zk_persistent_lock import zkPersistentLock
from grpc_microservice.room_server.zk_server.zk.zk_server_content import ServerContent
...
class ServerInspecte(metaclass=Singleton):
    ...
    def read_state(self):
        for node_name in ServerContent.get_list().keys():
            data = IKazooClient().get_kazoo_client().get("/".join([self.BIZ_PATH, self.WEBSOCKET, node_name]))[0].decode(
                "utf-8")
